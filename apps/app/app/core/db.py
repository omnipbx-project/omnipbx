...
def initialize_schema() -> None:
    ...
    with psycopg.connect(settings.db_dsn, autocommit=True) as connection:
        with connection.cursor() as cursor:
            ...
            cursor.execute("UPDATE extensions SET codecs = 'ulaw,alaw,opus' WHERE transport = 'transport-udp-softphone' AND (codecs IS NULL OR codecs = '' OR codecs = 'ulaw,alaw' OR codecs = 'ulaw,alaw,g722' OR codecs = 'g722,ulaw,alaw' OR codecs = 'opus,g722,ulaw,alaw' OR codecs = 'opus,ulaw,alaw')")
            cursor.execute("UPDATE extensions SET codecs = 'ulaw,alaw,opus' WHERE transport = 'transport-wss' AND (codecs IS NULL OR codecs = '' OR codecs = 'ulaw' OR codecs = 'ulaw,alaw' OR codecs = 'ulaw,alaw,g722' OR codecs = 'g722,ulaw,alaw' OR codecs = 'opus,g722,ulaw,alaw' OR codecs = 'opus,ulaw' OR codecs = 'opus,ulaw,alaw')")
