...
def sync_asterisk_config(connection: psycopg.Connection, reload_config: bool = True) -> dict[str, str | int]:
    ...
    dialplan_text = render_extensions_config(extensions, call_routing_rules)
    pjsip_trunks_text = render_trunk_pjsip_config(trunks)
    trunks_dialplan_text = render_trunk_dialplan(trunks, call_routing_rules)
...
def render_trunk_dialplan(trunks: list[dict], call_routing_rules: list[dict] | None = None) -> str:
    blocks = [
