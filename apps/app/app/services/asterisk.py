...
def sync_asterisk_config(connection: psycopg.Connection, reload_config: bool = True) -> dict[str, str | int]:
    ...
    pjsip_text = render_pjsip_config(extensions, pjsip_network) + _custom_config_text(advanced_custom_config, "pjsip")
...
def render_pjsip_config(extensions: list[dict], network: dict | None = None) -> str:
    blocks = ["; This file is generated by OmniPBX.\n"]
    advertised_host = _pjsip_advertised_host(network or {})
    media_address_line = f"media_address = {advertised_host}\n" if advertised_host else ""
